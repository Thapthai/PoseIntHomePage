...
const fadeClass = "animate-[fade-up_0.7s_ease-out_both]";
  ...
  return (
    ...
    >
      ...
      <div className="relative z-10 max-w-[1320px] mx-auto px-4 sm:px-6 flex flex-col items-center text-center pt-24 pb-32">
        <div className={`mb-6 ${fadeClass}`} style={delay(0)}>
          <img src="/Images/logo/POSE-LOGO.png" alt="POSE Intelligence" width={280} />
        </div>

        <div
          className={`inline-flex items-center gap-2 bg-white rounded-full px-4 py-1.5 mb-6 text-xs uppercase tracking-widest text-slate-800 font-semibold border border-accent/30 shadow-md ${fadeClass}`}
          style={delay(0.12)}
        >
        ...
        <h1
          className={`hero-title text-4xl sm:text-5xl md:text-6xl font-extrabold uppercase leading-tight max-w-4xl m-0 ${fadeClass}`}
          style={delay(0.22)}
        >
          {t.hero.titleBefore}{" "}
        ...
        <p
          className={`hero-readable mt-5 text-lg md:text-xl text-slate-800 font-medium max-w-2xl leading-relaxed ${fadeClass}`}
          style={delay(0.32)}
        >
          {t.hero.subtitle}
        </p>
        ...
        <div
          className={`flex flex-wrap justify-center gap-2 mt-6 ${fadeClass}`}
          style={delay(0.42)}
        >
        ...
        <div
          className={`flex flex-col sm:flex-row items-center gap-4 mt-10 ${fadeClass}`}
          style={delay(0.52)}
        >
          <a
            href="#about"
          ...
          >
            <span className="flex items-center justify-center w-8 h-8 rounded-full bg-accent/15 group-hover:bg-accent/25 transition-colors">
              <svg
                className="w-3.5 h-3.5 ml-0.5 text-accent"
                viewBox="0 0 24 24"
                fill="currentColor"
                aria-hidden
              >
      ...
      <a
        href="#about"
        className={`absolute bottom-8 left-1/2 -translate-x-1/2 z-10 flex flex-col items-center gap-1 text-slate-700 hover:text-slate-900 transition-colors hero-readable ${fadeClass}`}
        style={delay(0.62)}
